...
public class RouteGenerator : IIncrementalGenerator
{
    ...
    private void Execute(SourceProductionContext context, (Compilation Left, ImmutableArray<ClassDeclarationSyntax> Right) compilationTuple)
    {
        ...
        var routesAndNamespacesDictionary = new Dictionary<string, string>();
        ...
        foreach (var route in routeNameList)
        {
            ...
            var routeNamespace = classSymbol?.ContainingNamespace.ToDisplayString() ?? string.Empty;

            routesAndNamespacesDictionary.Add(route, routeNamespace);
        }

        AddExtraRoutes(context, compilation, classes, routeNameList, routesAndNamespacesDictionary);

        var source = BuildSource(routeNameList, routesAndNamespacesDictionary, namespaceName);

        context.AddSource(Constants.RoutesGenFileName, source);
    }

    private void AddExtraRoutes(SourceProductionContext context, Compilation compilation, ImmutableArray<ClassDeclarationSyntax> classes, ICollection<string> routeNameList, Dictionary<string, string> routesAndNamespacesDictionary)
    {
        ...
        foreach (var attributeData in classesWithExtraRouteAttributeData.SelectMany(classWithExtraRouteAttributeData => classWithExtraRouteAttributeData.AttributeData))
        {
            ...
            var extraRouteNamespace = classSymbol?.ContainingNamespace.ToDisplayString() ?? string.Empty;

            routesAndNamespacesDictionary.Add(extraRoute, extraRouteNamespace);
        }
    }
        ...
        var source = $$"""
                       ...
                       namespace {{namespaceName}}
                       {
                           public static class Routes
                           {
                               ...
                               {
                                   {{string.Join(",\n            ", routesAndNamespacesDictionary.Select(x => $"{{\"{x.Key}\",\"{x.Value}.{x.Key}\"}}"))}}
                               };
    ...
    private static string BuildExtraRouteAttribute()
    {
        return """
               ...
               public class ExtraRouteAttribute : Attribute
               {
                   ...
                   public ExtraRouteAttribute(string route, string typename = "")
                   {
                       Route = route;
                       Typename = typename;
                   }
